...
- Quota `consume` is called from API gateway middleware before request is forwarded to downstream services.
- Middleware flow: create/attach `request_id` UUID -> call `/quota/consume` -> if allowed, forward request; if rejected, return quota error immediately.
- If downstream work fails after a successful consume, middleware (or a retry/reconciliation job owned by the gateway) calls `/quota/refund` with the same `request_id`.
- For external client retries, the gateway can accept a client idempotency key and map it to the same internal `request_id`; without a stable retry key, a retried HTTP request may be treated as a new operation and consume quota again.
